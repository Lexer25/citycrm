...
class Controller_Companies extends Controller_Template
{
	...
	*/
	...
	public function action_save()//добавление организации
	{
		...
		if ($id == 0) {
			$id = $company->save($name, $code, $access, $parent, $group);
			$this->session->set('alert', __('company.saved'));
		} else {
			$company->update($id, $name, $parent, $code, $access, $group);
	...
	{
		$id=$this->request->param('id');
		Model::factory('Company')->delete($id);
		$this->session->set('alert', __('company.deleted'));
