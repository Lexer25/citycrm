...
<div class="onecolumn">
	...
	<div class="content">
		...
		<table id="tablesorter" class="table table-striped table-hover table-condensed tablesorter">
			...
			$aaa=array_chunk($accessNameList, $column);
			...
			for ($i=0; $i<$row; $i++)
			{
				echo '<tr>';
					foreach(Arr::get($aaa, $i) as $key=>$value)
					{
						...
						echo '<td>'.Form::checkbox('aclList['.Arr::get($value, 'ID_ACCESSNAME').']', 1, in_array (Arr::get($value, 'ID_ACCESSNAME'), $res)).' '. iconv('CP1251', 'UTF-8', Arr::get($value, 'NAME', '')).'</td>';
